...
/*   Updated: 2020/07/17 14:50:10 by jevan-de      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int			get_width_len(t_core *core, char *str, int strlen, int len)
{
	...
	if (precision)
	{
		if (precision->param > len)
			return (-1);
		preclen = (precision->param > strlen) ? precision->param - strlen : 0;
		if ((preclen + len + get_prefix_len(core, str)) <=
				((ft_strcmp(str, "0") == 0) ? 0 : strlen))
			return (-1);
		len -= preclen;
	}
	if (core->cspec == 'p')
		len -= ((strlen + 2) < len) ? (strlen + 2) : len;
	else
	{
		len -= (strlen + get_prefix_len(core, str));
		len += (is_zero_edge(core, str)) ? 1 : 0;
	}
	return (len);
}
